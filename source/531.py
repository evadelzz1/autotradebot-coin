...
startBalance = upbit.get_balance() # "KRW"    
print(startBalance)
while True:
    try:
        now = dt.datetime.now()        
        startTime = now - dt.timedelta(hours=1)
        endTime = now + dt.timedelta(hours=10)
        ...
        if startTime < now < endTime:
            ...
            SMA01, SMA02, SMA03, MACD, Signal = get_SMA_MACD(ticker)
            ...
            # 매수가 이루어지면 매수 당시의 현재가를 “buyPrice” 변수에 저장하고 print 문으로 한번 찍어준다.
            ...
            # print(f'[{formatedNow}] Price: {currentPrice:12.3f}, MACD: {MACD.iloc[-1]:15.5f}, SMA01: {SMA01.iloc[-1]:15.5f}, SMA02: {SMA02.iloc[-1]:15.5f}, SMA03: {SMA03.iloc[-1]:15.5f}, Signal: {Signal.iloc[-1]:15.5f}')
            print(f'[{formatedNow}] Price: {currentPrice:12.3f}, MACD: {MACD_:15.5f}, SMA01: {SMA01_:15.5f}, SMA02: {SMA02_:15.5f}, SMA03: {SMA03_:15.5f}, Signal: {Signal_:15.5f}, buyPrice: {buyPrice:12.3f}, sellPrice: {sellPrice:12.3f}, uuid: {actionUuid}')

        # 잔고 조회를 해서 자동매매를 시작했을 때의 가격보다 7% 이득을 봤다면 프로그램 종료
        lastBalance = upbit.get_balance("KRW")
        if lastBalance >= (startBalance * 1.07):
            print(f'[{formatedNow}] lastBalance: {lastBalance}, startBalance: {startBalance}')
            break
